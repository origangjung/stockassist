...
- 종목명·티커·한글 별칭 검색 후 현재가·차트·AI 분석이 함께 전환되는 동적 시장 화면
...
- 브라우저 로컬 관심 종목, 가격 참고 알림, 관리자 화면
- 서버 전용 분석 키와 읽기 전용 경로 허용목록을 사용하는 시장 데이터 BFF
- 관리자 미리보기와 고정 허용목록을 사용하는 운영 데이터 보존·정리
- Provider·보정 기준·규칙 버전별 캔들 현황과 자동 분류 차단 사유를 보여주는 읽기 전용 운영 화면
...
처음에는 `.env`의 `STOCK_PROVIDER=mock`, `AI_REPORT_PROVIDER=mock`을 유지하면 외부 API 키와
호출 비용 없이 사용할 수 있습니다. 실제 Provider를 사용할 때만 해당 자격증명을 서버
환경변수에 설정합니다. `.env` 파일과 API 키는 저장소에 커밋하면 안 됩니다.
...
```powershell
# 최초 실행 또는 node_modules가 없을 때만: corepack pnpm install --frozen-lockfile
pnpm --filter @stockpilot/web dev
```

첫 화면은 검색 전용입니다. 삼성전자·`005930`·`삼전`처럼 종목명, 티커 또는 등록된 한글
별칭을 입력하면 후보를 선택할 수 있고, 선택 뒤에만 멀티 에이전트 결과 화면이 위에서
아래로 나타납니다. 최근 검색과 관심 종목은 브라우저에만 저장되며, 관심 종목은 검색 뒤
결과 화면에서 다시 열거나 제거할 수 있습니다. 브라우저 저장소가 차단된 경우 관심 종목은
현재 탭에서만 유지됩니다.
`?symbol=005930` 주소와 결과 공유 링크는 같은 종목의 결과 화면으로 바로 연결됩니다.
...
기능 MVP 진행률은 약 **88~92%**, 실제 외부 Provider와 장시간 장애 시험까지 포함한 운영
준비도는 약 **70~75%**입니다.
...
| Phase 8~11 | MVP 완료 | Toss·DART·뉴스/공시·국내 수급 Provider 구조와 Mock 계약 |
...
캔들 보정 기준 화면에서는 Provider 출처와 버전화된 분류 규칙을 확인하고, 근거가 부족한
`unknown` 및 legacy 행을 수정 없이 식별할 수 있습니다. 각 그룹에는 원본 Provider,
계약·응답 근거, 엔드포인트 보정 의미, 계약 테스트와 규칙 버전 중 어떤 근거가 부족한지도
체크리스트로 표시됩니다.
...
Phase 8~15의 기능 경로는 MVP로 구현되어 있지만, 실계정 장중 장시간 연결·장애 복구·대규모
데이터 검증은 별도의 데스크톱 환경에서 수행해야 합니다.
